...
outputFileName = "schedule-files/Badfallsched.csv"
...
# important constants
numberOfActualSlotsInASlot = 4
timeSlotLengthInMinutes = 7
...
for line in availabilityFormResponses:
    print nameArray[i]
    auditioneeAvailability = line.split("\t")
    for day in xrange(len(days)):
        print "\tday: {}".format(days[day])
        print "\tavail: {}".format(auditioneeAvailability[day])
        ...
        for availability in totalTimeSlots[day]:
            if nameArray[i] in scheduledPeople:
                ...
                break
            elif availability in totalTimeSlots[day] and fill[day][j] < numberOfActualSlotsInASlot and auditioneeAvailability[day].find(availability) != -1:
...
with open(outputFileName, 'wb') as csvfile:
    auditionGenerator = csv.writer(csvfile, delimiter="\t")
    for key in output:
